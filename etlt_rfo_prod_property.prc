create or replace procedure u1.ETLT_RFO_PROD_PROPERTY is
s_mview_name     varchar2(30) := 'T_RFO_Z#PROD_PROPERTY';
   ...
   begin
      ...
      from s01.s$Z#PROD_PROPERTY@rdwh_exd;
   select last_date
      into d_src_commit_date_last
      from t_rdwh_increment_tables_load
     where object_name = 'T_RFO_Z#PROD_PROPERTY';

     delete from T_RFO_Z#PROD_PROPERTY
     where id in (select distinct id
                    from s01.s$Z#PROD_PROPERTY@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    insert into T_RFO_Z#PROD_PROPERTY
    select t.*
      from s01.Z#PROD_PROPERTY@rdwh_exd t
     where id in (select distinct id
                    from s01.s$Z#PROD_PROPERTY@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    commit;
