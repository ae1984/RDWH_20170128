create or replace force view u1.v_rdboard_online_alm_sale_day as
                  ...
                  from (select /*tm.yyyy_mm_dd,*/
                          ...
                          left join m_rfo_online_fld_ekt_day f
                            on trunc(f.folder_date_create_mi, 'mi') =
                               tm.yyyy_mm_dd_hh_mi
                           and (f.cr_program_name ='ЭКСПРЕСС-КРЕДИТОВАНИЕ-ТОВАР'   or f.cr_program_name ='ЭКСПРЕСС-КРЕДИТЫ (УСЛУГИ)') /*AND F.IS_CREDIT_ISSUED=1 */
...
;
grant select on U1.V_RDBOARD_ONLINE_ALM_SALE_DAY to LOADDB;
grant select on U1.V_RDBOARD_ONLINE_ALM_SALE_DAY to LOADER;
grant select on U1.V_RDBOARD_ONLINE_ALM_SALE_DAY to LOAD_RDWH_PROD;
