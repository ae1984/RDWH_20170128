create or replace force view u1.v_dsm_manager_folders_for_vid as
...
/*      and f.folder_date_create >= to_date('01/04/2014 00:00:00', 'dd/mm/yyyy hh24:mi:ss') -- дата начала периода
      ...
      and f.folder_date_create > trunc(sysdate) - 65
   )
group by trunc(cm.c_date_create), u.c_name, u.c_username, ca.c_value, sd.c_code || ' ' || sd.c_name, u.id, u.c_num_tab
--)--131677 -- 138272
;
grant select on U1.V_DSM_MANAGER_FOLDERS_FOR_VID to DSM;
grant select on U1.V_DSM_MANAGER_FOLDERS_FOR_VID to LOADDB;
grant select on U1.V_DSM_MANAGER_FOLDERS_FOR_VID to LOADER;
