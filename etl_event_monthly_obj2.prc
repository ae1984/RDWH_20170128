create or replace procedure u1.ETL_EVENT_MONTHLY_OBJ2 is
   v_event varchar2(50) := 'ETL_EVENT_MONTHLY_OBJ2';
...
begin
  ...
  select count(distinct l.object_name) into v_cnt from update_log l where l.end_refresh>=v_dt2 and l.object_name in ('V_FOLDER_ALL') and l.status='OK';
