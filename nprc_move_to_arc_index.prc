create or replace procedure u1.NPRC_MOVE_TO_ARC_INDEX is
begin
  ...
  from (
      select 'execute immediate (''ALTER INDEX '||tt.index_name||' REBUILD TABLESPACE USERS_ARC PARALLEL 30''); ' as sql1
      from
      (
          select distinct i.index_name 
          from user_indexes i
          left join NT_LOCKEDOBJ_HIST l on l.object_name=i.table_name and l.sdt+(3/24/60) >= sysdate
          where i.tablespace_name <> 'USERS_ARC' and i.table_owner='U1'
                and i.table_name in (select table_name from user_all_tables where tablespace_name='USERS_ARC')
