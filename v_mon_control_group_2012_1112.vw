create or replace force view u1.v_mon_control_group_2012_1112 as
with c as (
...
--         c2.prev_con_prod_first_by_std
  ...
  c2.yy_mm_start_first in ('2012 - 11','2012 - 12'/*,'2013 - 01','2013 - 02'*/)
), p as (
  ...
  from v_portfolio p where p.report_month_all_contracts = (select max(d7.YY_MM_REPORT) from v_data_all d7)
                ...
                /*mr.FOLDER_ID,*/ max(mr.REJ_CAUSE_CODE) keep (
                          dense_rank first order by mr.RES_DATE) as REJ_CAUSE_CODE, c1.rnn/*, c1.rnn*/
...
join r on r.rnn = c.client_rnn_first and r.RES_DATE = c.start_date_first
--group by c.yy_mm_start_first
) x
;
grant select on U1.V_MON_CONTROL_GROUP_2012_1112 to LOADDB;
grant select on U1.V_MON_CONTROL_GROUP_2012_1112 to LOADER;
