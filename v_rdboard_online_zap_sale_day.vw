create or replace force view u1.v_rdboard_online_zap_sale_day as
                  ...
                  from (select /*tm.yyyy_mm_dd,*/
                          ...
                          left join m_rfo_online_fld_ekt_day f
                           ...
                           and (f.cr_program_name =
                               ...
                               'ЭКСПРЕСС-КРЕДИТЫ (УСЛУГИ)' ) /*AND F.IS_CREDIT_ISSUED=1*/ and f.x_dnp_region='ЗАПАД'
...
;
grant select on U1.V_RDBOARD_ONLINE_ZAP_SALE_DAY to LOADDB;
grant select on U1.V_RDBOARD_ONLINE_ZAP_SALE_DAY to LOADER;
grant select on U1.V_RDBOARD_ONLINE_ZAP_SALE_DAY to LOAD_RDWH_PROD;
