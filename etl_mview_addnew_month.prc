create or replace procedure u1.ETL_MVIEW_ADDNEW_MONTH is
begin
  ...
  select N_TASKS_EXECUTE_SEQ.nextval
         ...
         ,'nprc_mviewrefresh_daily('''||ttt.object_name||'''); '||nvl(ttt.check_proc,'null')||';'
         ,'NEW'
         ,ttt.object_name
  from (
    select 
  ...
  where ttt.cnt_ref_obj <= ttt.cnt_ref_obj_ready and ttt.cnt_event<=ttt.cnt_event_ready and rownum <= nfnc_get_task2exec
      /*and 700>=(select value from V$PX_PROCESS_SYSSTAT t where t.STATISTIC = 'Servers In Use                ')
