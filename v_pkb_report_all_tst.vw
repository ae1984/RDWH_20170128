create or replace force view u1.v_pkb_report_all_tst as
select /*+first_rows*/
    p.id as report_id,
...
left join (select s1.collection_id,
                  min(s1.id) as id,
                  listagg(s1.c_value,'#') within group (order by s1.c_value) as c_value
             from V_RFO_Z#KAS_STRING_250 s1
            group by s1.collection_id) s on s.collection_id = p.c_agre_statuses and p.c_rpt_type  = 2
join V_RFO_Z#FDOC fd on fd.id = p.id and fd.class_id = 'PKB_REPORT'
;
grant select on U1.V_PKB_REPORT_ALL_TST to LOADDB;
