create or replace force view u1.v_contract_folder as
select /*+first_rows*/
       to_number(replace(replace(replace(replace(c.contract_number,'/CRD',''),'-',''),'R',7),'e',8)) as co_contract_number_num,
...
;
grant select on U1.V_CONTRACT_FOLDER to LOADDB;
grant select on U1.V_CONTRACT_FOLDER to LOADER;
