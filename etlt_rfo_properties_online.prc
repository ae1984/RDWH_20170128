create or replace procedure u1.ETLT_RFO_PROPERTIES_ONLINE is
s_mview_name     varchar2(30) := 'T_RFO_Z#PROPERTIES_ONLINE';
   ...
   select last_date
     ...
     where object_name = 'T_RFO_Z#PROPERTIES_ONLINE';

     delete from T_RFO_Z#PROPERTIES_ONLINE
     where id in (select distinct id
                    from s01.s$Z#PROPERTIES@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    insert into T_RFO_Z#PROPERTIES_ONLINE
    select t.*
      from s01.Z#PROPERTIES@rdwh_exd t
     where id in (select distinct id
                    from s01.s$Z#PROPERTIES@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    commit;
    ...
    update t_rdwh_increment_tables_load
     ...
     where object_name = 'T_RFO_Z#PROPERTIES_ONLINE';
    commit;
 end ETLT_RFO_PROPERTIES_ONLINE;
/
