create or replace procedure u1.NPRC_TASKS_EXECUTEID2(p_task_id number) is
/*
    alexey.yevseyev
    20/10/2016
...
begin
  ...
  v_thread_num_min:=5; --мин. кол-во исполняемых потоков
  ...
  for rec in (select
      ...
      if v_group_num<>rec.group_num  then --если изменился номер группы в рамках задачи
         ...
         loop
           dbms_lock.sleep(3);
           select count(1) into v_num_tmp from USER_SCHEDULER_JOBS /*ALL_SCHEDULER_RUNNING_JOBS*/ where job_name like v_jobprefix||'%';
           select count(1) into v_num_tmp2 from ALL_SCHEDULER_RUNNING_JOBS where job_name like v_jobprefix||'%';
         ...
         select count(1) into v_num_tmp from NT_TASKS_EXECUTE
      ...
      if v_status = /*'NEW'*/ v_first_status then
          ...
          if i>=v_thread_num then --если запущеные потоки достигли допустимого максимума
            ...
            loop
              ...
              select count(1) into i from USER_SCHEDULER_JOBS where job_name like v_jobprefix||'%'; --проверяем число активных сессий(JOB_RT) на текущий момент и изменяем счетчик потоков
