create or replace procedure u1.ETLT_RBO_Z#PLAN_OPER is
       s_mview_name     varchar2(30) := 'T_RBO_Z#PLAN_OPER';
      ...
      begin
        select max(src_commit_date)
          ...
          from s02.s$Z#PLAN_OPER@rdwh_exd;
        ...
        --
        delete from T_RBO_Z#PLAN_OPER
         where id in (select distinct id
                        from s02.s$Z#PLAN_OPER@rdwh_exd
                       where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
        insert /*+ append */into T_RBO_Z#PLAN_OPER
        select *
          from s02.Z#PLAN_OPER@rdwh_exd
         where id in (select distinct id
                        from s02.s$Z#PLAN_OPER@rdwh_exd
                       where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
        commit;
