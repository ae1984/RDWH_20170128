create or replace procedure u1.ETLT_RFO_KAS_CANCEL
  is
   s_mview_name     varchar2(30) := 'T_RFO_Z#KAS_CANCEL';
  ...
  begin
    ...
    select max(src_commit_date)-5/24/60/60
      ...
      from s01.s$Z#KAS_CANCEL@rdwh_exd;

    select last_date
      into d_src_commit_date_last
      from t_rdwh_increment_tables_load
     where object_name = 'T_RFO_Z#KAS_CANCEL';
    --
    delete from T_RFO_Z#KAS_CANCEL
     where id in (select distinct id
                    from s01.s$Z#KAS_CANCEL@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    insert into T_RFO_Z#KAS_CANCEL
    select t.*
      from s01.Z#KAS_CANCEL@rdwh_exd t
     where id in (select distinct id
                    from s01.s$Z#KAS_CANCEL@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    commit;
