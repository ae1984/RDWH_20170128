create or replace force view u1.v_sys_sessions_active as
...
) a left join sys.v_$sqltext st on st.SQL_ID = a.sql_id and st.PIECE in (0, 1, 2, 3)
...
order by 1, 2, 3 desc;
grant select on U1.V_SYS_SESSIONS_ACTIVE to LOADDB;
grant select on U1.V_SYS_SESSIONS_ACTIVE to LOADER;
