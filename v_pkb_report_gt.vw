create or replace force view u1.v_pkb_report_gt as
select /*+ parallel 30*/
    p.id as report_id,
...
join V_RFO_Z#FDOC fd on  fd.id = p.id
                     and fd.class_id = 'PKB_REPORT'
left join V_RFO_Z#KAS_PKB_CI c on c.collection_id = p.c_ci
...
where p.c_rpt_type = 2
  and p.id in (select id from T_PKB_REPORT_GT)
;
grant select on U1.V_PKB_REPORT_GT to LOADDB;
