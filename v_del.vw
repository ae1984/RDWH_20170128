create or replace force view u1.v_del as
select --+ noparallel
...
where --(c.yy_mm_start_last like '2010 - %' or c.yy_mm_start_last like '2011 - %' or c.yy_mm_start_last like '2012 - %' or
(c.yy_mm_start_last like '2013 - %' or (c.yy_mm_start_last like '2014 - %' and c.yy_mm_start_last != '2014 - 11')) and
      (p.pos_type = 'ОТДЕЛЕНИЕ'
       --     dc.prod_type in ('АВТОКРЕДИТОВАНИЕ','АВТОКРЕДИТОВАНИЕ БУ','АВТОЗАПЧАСТИ')
       )-- and
  --    p.x_dnp_name = 'АЛМАТЫ' and
-- and    p.pos_code = '001-116-461' --and
...
;
grant select on U1.V_DEL to DNP;
grant select on U1.V_DEL to LOADDB;
grant select on U1.V_DEL to LOADER;
