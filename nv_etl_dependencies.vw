create or replace force view u1.nv_etl_dependencies as
select zz.owner, zz.name, zz.referenced_name from (
  select distinct owner, name, referenced_name from /*SYS.DBA_DEPENDENCIES*/ NT_DBA_DEPENDENCIES where owner='U1' and referenced_owner='U1' and referenced_link_name is null
  union
...
grant select on U1.NV_ETL_DEPENDENCIES to LOADDB;
