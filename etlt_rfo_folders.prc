create or replace procedure u1.ETLT_RFO_FOLDERS is
   s_mview_name     varchar2(30) := 'T_RFO_Z#FOLDERS';
  ...
  begin
    ...
    select max(src_commit_date)-5/24/60/60
      ...
      from s01.s$Z#FOLDERS@rdwh_exd;
    select last_date
      into d_src_commit_date_last
      from t_rdwh_increment_tables_load
     where object_name = 'T_RFO_Z#FOLDERS';
    --
    delete from T_RFO_Z#FOLDERS
     where id in (select  id
                    from s01.s$Z#FOLDERS@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    insert into T_RFO_Z#FOLDERS
    select *
      from s01.Z#FOLDERS@rdwh_exd
     where id in (select  id
                    from s01.s$Z#FOLDERS@rdwh_exd
                   where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
    commit;
