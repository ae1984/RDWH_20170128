create or replace procedure u1.etleml_cpr_dep1
  as
    EMAIL_CODE_DEP1 varchar2(32) := 'FOLDER_CPR_DEP1';
  ...
  begin
    ...
    v_error_count := 0;
    v_subject := 'Оформление доверенности на распоряжение депозитом';
    ...
    loop
      ...
      v_message := v_message || r.folder_id || TAB1 || to_char(r.folder_date_create, 'dd-mm-yyyy hh24:mi:ss') || TAB1 ||
                   r.folder_number || TAB1 || r.folder_state || TAB1 || r.business_process || CRLF1;

      insert into u1.T_FOLDER_CPR_DEP1_SENDED (folder_id, date_create)
      values (r.folder_id, sysdate);
    end loop;

    if (v_error_count > 0) then
      pkg_mail.add_email(in_email_code => EMAIL_CODE_DEP1,
