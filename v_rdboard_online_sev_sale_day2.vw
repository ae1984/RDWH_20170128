create or replace force view u1.v_rdboard_online_sev_sale_day2 as
                  ...
                  from (select /*tm.yyyy_mm_dd,*/
                          ...
                          left join m_rfo_online_fld_ekt_day f
                           ...
                           and (f.cr_program_name =
                               ...
                               'ЭКСПРЕСС-КРЕДИТЫ (УСЛУГИ)' ) AND F.IS_CREDIT_ISSUED=1 and f.is_point_active=0 and f.x_dnp_region='СЕВЕР'
...
;
grant select on U1.V_RDBOARD_ONLINE_SEV_SALE_DAY2 to LOADDB;
grant select on U1.V_RDBOARD_ONLINE_SEV_SALE_DAY2 to LOADER;
grant select on U1.V_RDBOARD_ONLINE_SEV_SALE_DAY2 to LOAD_RDWH_PROD;
