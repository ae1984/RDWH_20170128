create or replace procedure u1.ETLT_RFO_Z_KAS_STRING_250
  is
   s_mview_name     varchar2(30) := 'T_RFO_Z#KAS_STRING_250';
  ...
  begin
   ...
   select max(src_commit_date)  into d_src_commit_date_load from s01.s$Z#KAS_STRING_250@rdwh_exd;

   select last_date-10/24/60/60 into d_src_commit_date_last
   from t_rdwh_increment_tables_load
   where object_name = 'T_RFO_Z#KAS_STRING_250';
   --
   delete from T_RFO_Z#KAS_STRING_250
   where id in (select distinct id
                from s01.s$Z#KAS_STRING_250@rdwh_exd
                where src_commit_date between d_src_commit_date_last and d_src_commit_date_load
               );
   insert into T_RFO_Z#KAS_STRING_250
   select *
   from s01.Z#KAS_STRING_250@rdwh_exd
   where id in (select distinct id
                from s01.s$Z#KAS_STRING_250@rdwh_exd
                where src_commit_date between d_src_commit_date_last and d_src_commit_date_load
               );
