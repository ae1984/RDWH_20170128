create or replace force view u1.v_rej_score_knp_bw_22_1_fld as
select folder_id,
...
from (select t.folder_id,
   ...
   case
      when pmt_ours is null  then 16
      when pmt_ours< 19052  then 4
      when 19052<= pmt_ours and pmt_ours< 19593  then 14
      when 19593<= pmt_ours and pmt_ours< 22634.5  then 4
      when 22634.5<= pmt_ours  then 7 end SCR_PMT_NOM_RATE,
...
from (select a.*, case when a.SEX = 'М' then a.AGE_FULL_YEARS*(-1) else a.AGE_FULL_YEARS end AGE_FULL_YEARS_GENDER from m_folder_con_miner_folder a
     where a.cr_program_name = 'КРЕДИТ НАЛИЧНЫМИ ПОВТОРНИКУ') t);
grant select on U1.V_REJ_SCORE_KNP_BW_22_1_FLD to LOADDB;
grant select on U1.V_REJ_SCORE_KNP_BW_22_1_FLD to LOADER;
