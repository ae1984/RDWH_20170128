create or replace procedure u1.ETLT_RFO_Z_KAS_PC_DOG
   is
       s_mview_name     varchar2(30) := 'T_RFO_Z#KAS_PC_DOG';
      ...
      begin
        ...
        select max(src_commit_date)
          ...
          from s01.s$Z#KAS_PC_DOG@rdwh_exd;
        ...
        delete from T_RFO_Z#KAS_PC_DOG
         where id in (select id
                        from s01.s$Z#KAS_PC_DOG@rdwh_exd
                       where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);

        insert /*+ append */into T_RFO_Z#KAS_PC_DOG
        select *
        from s01.Z#KAS_PC_DOG@rdwh_exd
        where id in (select id
                     from s01.s$Z#KAS_PC_DOG@rdwh_exd
                     where src_commit_date between d_src_commit_date_last and d_src_commit_date_load);
        commit;
