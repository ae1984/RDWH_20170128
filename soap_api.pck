CREATE OR REPLACE PACKAGE U1.soap_api AS
-- --------------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY U1.soap_api AS
...
BEGIN
  IF g_debug THEN
    ...
    END IF;
    dbms_output.put_line('LENGTH(p_env='||LENGTH(p_env));
...
-- ---------------------------------------------------------------------
  l_envelope       clob; --varchar2(32767); --clob;
  l_envelope_back  varchar2(32000); 
...
BEGIN
 ...
 -- UTL_HTTP.set_detailed_excp_support(ENABLE => TRUE );
  ...
  --UTL_HTTP.set_transfer_timeout(l_http_request, 1000);
  utl_http.set_authentication(l_http_request, 'WebServiceDWHR', 'WebServiceDWHR12345',for_proxy => false); -- Use HTTP Basic Authen. Scheme
  ...
  loop
DBMS_LOB.READ(l_envelope,amt,pos,l_envelope_back);
UTL_HTTP.WRITE_TEXT(l_http_request, l_envelope_back); 
EXIT WHEN amt < 4000;
pos := pos + amt;
amt := 4000;
END loop;
...
/*  UTL_HTTP.write_text(l_http_request, l_envelope);
 ...
 l_http_response := UTL_HTTP.get_response(l_http_request);
  ...
  begin
  loop
   UTL_HTTP.READ_TEXT(l_http_response, l_envelope_back);
   cl_envelope := cl_envelope || l_envelope_back;
  END LOOP; 
...
END;
  ...
  UTL_HTTP.end_response(l_http_response);
--  dbms_output.put_line('LENGTH(l_envelope)='||utl_raw.length(l_envelope));
  --show_envelope(utl_raw.cast_to_varchar2(l_envelope), 'Response');
  --show_envelope(l_envelope, 'Response');
  l_response.doc := XMLTYPE.createxml(cl_envelope);  
...
BEGIN
  ...
  l_http_request := UTL_HTTP.begin_request(p_url, 'POST','HTTP/1.1');
  UTL_HTTP.set_transfer_timeout(l_http_request, p_timeout);
  UTL_HTTP.set_header(l_http_request, 'Content-Type', 'text/xml');
  UTL_HTTP.set_header(l_http_request, 'Content-Length', LENGTH(l_envelope));
  UTL_HTTP.set_header(l_http_request, 'SOAPAction', p_action);
  UTL_HTTP.write_text(l_http_request, l_envelope);
