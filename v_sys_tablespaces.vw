CREATE OR REPLACE FORCE VIEW U1.V_SYS_TABLESPACES AS
SELECT ts.tablespace_name, status,
DECODE(dt.contents,'PERMANENT',DECODE(dt.extent_management,'LOCAL',DECODE(
       dt.allocation_type,'UNIFORM','LM-UNI','LM-SYS'),'DM'),'TEMPORARY','TEMP',dt.contents) ts_type,
NVL(s.count,0) objects,
ts.files,
ts.allocated/1024/1024 allocated_mb,
ROUND((ts.allocated-nvl(ts.free_size,0))/1024/1024) used_mb,
ROUND(maxbytes/1024/1024) max_size_mb,
ROUND((maxbytes-(ts.allocated-nvl(ts.free_size,0)))/1024/1024) max_free_mb,
ROUND((maxbytes-(ts.allocated-nvl(ts.free_size,0)))*100/maxbytes,1) free_pct
FROM
(
...
ORDER BY 1;
grant select on U1.V_SYS_TABLESPACES to LOADDB;
grant select on U1.V_SYS_TABLESPACES to LOADER;
